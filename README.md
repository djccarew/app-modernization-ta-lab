...
1. Lets's look at the apps designated with red to indicate that changes to the app are required before running the app on WebSphere. Click on the link for the app **petstore-WAS.ear**. This is a JavaEE 5 app written by the Java BluePrints program at Sun Microsystems (later acquired by Oracle).
...
### Step 3: Build a WebSphere Liberty image of the app and push it to the ICP private repository
...
2. From a terminal window log in to the ICP Cluster with the following command (Note substitute your ICP Master IP for **[ICP Master IP]**)
```
    cloudctl login -a [ICP Master IP]:8443 --skip-ssl-validation
```
3. Clone the Git repo that contains the Plants By WebSphere app + the artifacts generated by Transformation Advisor by typing in (or copying and pasting in) the following commands
```
  git clone https://github.com/djccarew/app-modernization-plants-by-websphere-jee6.git
  cd app-modernization-plants-by-websphere-jee6

```
4. Build a docker image  by typing in (or copying and pasting in) the following (uncommented) commands
```
...
```
5. Push the image to the ICP private repository by typing in (or copying and pasting in) the following (uncommented) commands
```
